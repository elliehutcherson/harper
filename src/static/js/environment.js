...
export class Environment {
   ...
   * @property {HTMLElement} totalSprinklesElement - Element to display total sprinkles
   * @property {HTMLElement} timeElement - Element to display elapsed time
   * @property {HTMLElement} sprinklesPerMinuteElement - Element to display SPM
   */
  constructor(config = {}) {
    // Initialize configuration with defaults
    this.config = {
      shopSelector: '.items-list',
      sprinkleCountSelector: '#stats-current-sprinkles',
      totalSprinklesSelector: '#stats-total-sprinkles',
      timeSelector: '#stats-elapsed-time',
      sprinklesPerMinuteSelector: '#stats-sprinkles-per-minute',
    ...
    this.totalSprinklesElement = document.querySelector(this.config.totalSprinklesSelector);
    this.timeElement = document.querySelector(this.config.timeSelector);
    this.sprinklesPerMinuteElement = document.querySelector(this.config.sprinklesPerMinuteSelector);
  ...
  updateUI() {
    ...
    if (this.totalSprinklesElement) {
      this.totalSprinklesElement.textContent = formattedTotalSprinkles;
    }
    
    // Update elapsed time
    if (this.timeElement) {
      const elapsedTime = Date.now() - this.startTime;
      this.timeElement.textContent = this.formatTime(elapsedTime);
    }
    
    // Update sprinkles per minute
    if (this.sprinklesPerMinuteElement) {
      const spm = this.shop.getTotalSprinklesPerMinute();
      this.sprinklesPerMinuteElement.textContent = this.formatDecimal(spm);
    }
  }
